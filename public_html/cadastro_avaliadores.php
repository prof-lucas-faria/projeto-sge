                        ...
                        <form class="needs-validation" id="formulario" data-evento_id="1">
                            ...
                            <div id="avaliadores">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control avaliador_id"
                                           placeholder="Insira o título da atividade"
                                           data-avaliador_id="<?= (isset($avaliador->avaliador_id)) ? $avaliador->avaliador_id : "" ?>"
                                           value="<?= (isset($avaliador->nome)) ? $avaliador->nome : "" ?>" required
                                           autofocus>
                                </div>

                                <div class="form-group">
                                    <label>Área de atuação:</label> <br>
                                    <select data-placeholder="Escolha as áreas de atuação"
                                            class="custom-select tematica" multiple>
                                        <option value=""></option>
                                        <?php
                                        foreach ($lista_tematicas as $key => $tematica) { ?>
                                            <option
                                                value="<?= $tematica->tematica_id ?>"> <?= $tematica->descricao ?> </option>
                                            <?php
                                        }
                                        ?>
                                    </select>
