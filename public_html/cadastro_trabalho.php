            ...
            <div class="row justify-content-md-center">
                <div class="col-md-9">
                    <form class="needs-validation" id="formulario"
                          data-evento_id="<?= isset($evento_id) ? $evento_id : '' ?>"
                                ...
                                <select data-placeholder="Escolha os tipos de trabalhos" class="custom-select" id="tipo">
                                    ...
                                    <?php foreach ($lista_tipos as $key => $tipo) { ?>
                                        <option value="<?= $tipo->tipo_id ?>"
                                            <?= (isset($trabalho->tipo_id) && $tipo->tipo_id == $trabalho->tipo_id) ? "selected" : "" ?>
                                            data-qtde_max_autor="<?= isset($tipo->qtde_max_autor) ? $tipo->qtde_max_autor : '0' ?>">
                                            <?= $tipo->descricao ?>
                                        </option>
                                    <?php } ?>
                                </select>
                                ...
                                <select data-placeholder="Escolha as áreas de atuação" class="custom-select tematica" id="tematica">
                                    ...
                                    <?php foreach ($lista_tematicas as $key => $tematica) { ?>
                                        <option value="<?= $tematica->tematica_id ?>"
                                            <?= (isset($trabalho->tematica_id) && $tematica->tematica_id == $trabalho->tematica_id) ? "selected" : "" ?>>
                                            <?= $tematica->descricao ?>
                                        </option>
                                    <?php } ?>
                                </select>
                                        ...
                                        <div class="input-group-prepend">
                                            <div class="input-group-text">
                                                <input type="checkbox" id='apresentadores' name='apresentadores'
                                                    <?= (isset($listaAutores[0]->apresentador) && $listaAutores[0]->apresentador == 1) ? 'checked' : '' ?>>
                                            </div>
                                        </div>
                                        <input type="text" class="form-control" id="autores" name="autores" placeholder=""
                                               data-usuario_id="<?= (isset($usuario->usuario_id)) ? $usuario->usuario_id : "" ?>"
                                               value="<?= (isset($usuario->nome)) ? $usuario->nome : "" ?>" disabled>
                                    </div>
                                    <?php
                                            ...
                                            for ($i = 1; $i < count((array) $listaAutores); $i++) { ?>
                                                <div class='input-group mt-3' name='autores'>
                                                    <div class='input-group-prepend'>
                                                        <div class='input-group-text'>
                                                            <input type='checkbox' id='apresentadores' name='apresentadores'
                                                                aria-label='Checkbox for following text inpu'
                                                                <?= (isset($listaAutores[$i]->apresentador) && $listaAutores[$i]->apresentador == 1) ? 'checked' : '' ?>>
                                                        </div>
                                                    </div>
                                                    <input type="text" class="form-control" id="autores" name="autores"
                                                           data-usuario_id="<?= (isset($listaAutores[$i]->usuario_id)) ? $listaAutores[$i]->usuario_id : "" ?>"
                                                           value="<?= (isset($listaAutores[$i]->nome)) ? $listaAutores[$i]->nome : "" ?>">
                                                </div>
                                            <?php }
                                        }
                                } else { ?>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                ...
                                <input type="text" class="form-control" id="titulo" placeholder="Insira o título "
                                       value="<?= (isset($trabalho->titulo)) ? $trabalho->titulo : "" ?>">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                                ...
                                <div class="input-group">
                                    ...
                                    <div class="col-md-2">
                                        <button class="btn btn-outline-secondary col-md-12" type="button"
                                                name='download_arquivo' id="download_arquivo_nao_identificado"
                                                data-path=<?= (isset($trabalho->arquivo_nao_identificado)) ? '"' . $trabalho->arquivo_nao_identificado . '"' : '""' . 'disabled=' . '"disabled"' ?>>
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <div class="input-group">
                                    ...
                                    <div class="col-md-2">
                                        <button class="btn btn-outline-secondary col-md-12" type="button" name='download_arquivo'
                                                id="download_arquivo_identificado" data-path=<?= (isset($trabalho->arquivo_identificado)) ? '"' . $trabalho->arquivo_identificado . '"' : '""' . 'disabled=' . '"disabled"' ?>>
                                            <i class="fa fa-download" aria-hidden="true"></i></button>
                                    </div>
                                </div>
